...
df.drop(['Student_ID', 'Relationship_Status'], inplace=True, axis=1, errors='ignore')

col1, col2 = st.columns(2, gap="small")
col3, = st.columns(1)   

with col1:
    count_female = df[(df['Age'] >=21) & (df['Gender']=="Female") & (df['Academic_Level']=="Graduate")].shape[0]
    st.write(f"The total count for Female Graduates of age more than or equal to 21 is: {count_female}")
    count_male = df[(df['Age'] >=21) & (df['Gender']=="Male") & (df['Academic_Level']=="Graduate")].shape[0]
    ...
    fig1 = px.pie(comaprison_df, names="Gender", values="Count",
                  title="Comparison between number of Male and Female Graduates")
...
with col3:
    ...
    mental = df.groupby(['Most_Used_Platform', 'Mental_Health_Score', 'Country']).size().reset_index(name='Count')
    ...
    hours = df.groupby(by='Country')['Sleep_Hours_Per_Night'].mean().reset_index()
    ...
    req_data.columns = ['Affects_Academic_Performance', 'count']
